          ...
          while True:
            try:
              ...
              if code == ServerMessage.EXIT.value:
                ...
                self.main_server.operations[code](self, buffer)
            except:
              traceback.print_exc()
...
class MegaposeServer():
  ...
  def __init__(self, host: str, port: int, model_name: str, mesh_dir: Path, camera_data: Dict,
    ...
    if self.warmup:
      ...
      observation = self._make_observation_tensor(self.camera_data, np.random.randint(0, 255, (h, w, 3), dtype=np.uint8),
  ...
  def _estimate_pose(self, client: ConnectionThread, buffer: io.BytesIO):
    '''
    Network method
    ...
    observation = self._make_observation_tensor(client.camera_data, img, depth)
    if self.device == 'cuda':
  ...
  def _raw_viz(self, client: ConnectionThread, buffer: io.BytesIO):
    json_object = json.loads(read_string(buffer))
    labels = json_object['labels']
  ...
  def _score(self, client: ConnectionThread, buffer: io.BytesIO):
    img = read_image(buffer)
    json_object = json.loads(read_string(buffer))
    ...
    observation = self._make_observation_tensor(client.camera_data, img)
    if self.device == 'cuda':
      observation = observation.cuda()
    with self.lock_megapose:
  ...
  def _list_objects(self, client: ConnectionThread, _buffer: io.BytesIO):
  ...
  def _make_observation_tensor(self, camera_data: CameraData, image: np.ndarray, depth: Optional[np.ndarray]=None) -> ObservationTensor:
    '''
    Create an observation tensor from an image and a potential depth image
    '''
    return ObservationTensor.from_numpy(image, depth, camera_data.K)


  def _set_intrinsics(self, client: ConnectionThread, buffer: io.BytesIO) -> None:
    '''
    Network method
    ...
    client.camera_data = self._make_camera_data({
      'K': K,
      'h': h,
      'w': w
    })
  ...
  def _set_SO3_grid_size(self, client: ConnectionThread, buffer: io.BytesIO):
    json_object = json.loads(read_string(buffer))
